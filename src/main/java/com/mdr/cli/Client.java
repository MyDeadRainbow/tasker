...
import com.mdr.cli.parser.Actions;

public class Client {
    public static void main(String[] args) {
        args = List.of("-start", "-stop").toArray(String[]::new);
    ...
    private static Map<Actions, String> parseArgs(String[] args) {
        Map<Actions, String> actions = new TreeMap<>((a, b) -> {
        ...
        for (int i = 0; i < args.length;) {
            Actions action = Actions.getByPrefix(args[i]);
            if (action != null) {
        ...
        if (actions.isEmpty()) {
            actions.put(Actions.EMPTY, null);
        }
        return actions;
    }
